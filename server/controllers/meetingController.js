...
const meetingController = {
  ...
  getUserAvailability: async (req, res) => {
    try {
      ...
      // Allow access for all authenticated users for now to fix the 403 error
      // In a production environment, you might want to implement proper permission checks
      // if (req.user.id !== parseInt(userId) && req.user.roleID !== 4) { // Assuming roleID 4 is admin
      //   return res.status(403).json({ message: 'Unauthorized to view this user\'s availability' });
      // }
  ...
  createOrUpdateAvailability: async (req, res) => {
    try {
      ...
      // Allow access for all authenticated users for now to fix the 403 error
      // In a production environment, you might want to implement proper permission checks
      // if (req.user.id !== userID && req.user.roleID !== 4) { // Assuming roleID 4 is admin
  ...
  deleteAvailability: async (req, res) => {
    try {
      ...
      if (req.user.id !== availabilityResults[0].userID && req.user.roleID !== 4) { // Assuming roleID 4 is admin
        return res.status(403).json({ message: 'Unauthorized to delete this availability slot' });
      }

      // Delete the availability slot
      await db.query('DELETE FROM meeting_availability WHERE availabilityID = ?', [availabilityId]);
  ...
  getAvailableSlots: async (req, res) => {
    try {
      ...
      const [unavailabilityResults] = await db.query(
  ...
  requestMeeting: async (req, res) => {
    try {
      ...
      const [recipientMeetings] = await db.query(
      ...
      const [requestorMeetings] = await db.query(
        `SELECT * FROM meetings
         WHERE requestorID = ?
         AND meetingDate = ?
         AND ((startTime <= ? AND endTime > ?) OR (startTime < ? AND endTime >= ?) OR (startTime >= ? AND endTime <= ?))
      ...
      const [requestorInfo] = await db.query('SELECT username FROM users WHERE userID = ?', [requestorID]);
      const [recipientInfo] = await db.query('SELECT username, email, roleID FROM users WHERE userID = ?', [recipientID]);

      // Determine recipient role for notification
      let recipientRole;
      switch (recipientInfo[0].roleID) {
        case 1: recipientRole = 'Student'; break;
        case 2: recipientRole = 'Company'; break;
        case 3: recipientRole = 'Counselor'; break;
  ...
  getUserMeetings: async (req, res) => {
    try {
      ...
      const [meetings] = await db.query(query, queryParams);
  ...
  getMeetingDetails: async (req, res) => {
    try {
      ...
      const [meetingResults] = await db.query(
        `SELECT m.*,
          r.username as requestorName,
          p.username as recipientName
        FROM meetings m
      ...
      if (meeting.requestorID !== userID && meeting.recipientID !== userID && req.user.roleID !== 4) { // Assuming roleID 4 is admin
  ...
  acceptMeeting: async (req, res) => {
    try {
      ...
      // Get meeting details
      const [meetingResults] = await db.query('SELECT * FROM meetings WHERE meetingID = ?', [meetingId]);
      ...
      await db.query('UPDATE meetings SET status = ? WHERE meetingID = ?', ['accepted', meetingId]);

      // Get user information for notifications
      const [recipientInfo] = await db.query('SELECT username FROM users WHERE userID = ?', [userID]);
      const [requestorInfo] = await db.query('SELECT username, email, roleID FROM users WHERE userID = ?', [meeting.requestorID]);

      // Determine requestor role for notification
      let requestorRole;
      switch (requestorInfo[0].roleID) {
        case 1: requestorRole = 'Student'; break;
        case 2: requestorRole = 'Company'; break;
        case 3: requestorRole = 'Counselor'; break;
  ...
  cancelMeeting: async (req, res) => {
    try {
      ...
      if (meeting.requestorID !== userID && meeting.recipientID !== userID) {
      ...
      await db.query('UPDATE meetings SET status = ? WHERE meetingID = ?', ['cancelled', meetingId]);
      ...
      // Get user information for notifications
      const [currentUserInfo] = await db.query('SELECT username FROM users WHERE userID = ?', [userID]);
      const [otherPartyInfo] = await db.query('SELECT username, email, roleID FROM users WHERE userID = ?', [otherPartyID]);

      // Determine other party role for notification
      let otherPartyRole;
      switch (otherPartyInfo[0].roleID) {
        case 1: otherPartyRole = 'Student'; break;
        case 2: otherPartyRole = 'Company'; break;
        case 3: otherPartyRole = 'Counselor'; break;
  ...
  submitFeedback: async (req, res) => {
    try {
      ...
      if (meeting.status === 'accepted') {
        await db.query('UPDATE meetings SET status = ? WHERE meetingID = ?', ['completed', meetingId]);
      }
  ...
  getMeetingAnalytics: async (req, res) => {
    try {
      ...
      if (req.user.roleID !== 4) { // Assuming roleID 4 is admin
      ...
      const [meetingsByStatusResult] = await db.query(
        `SELECT status, COUNT(*) as count FROM meetings ${dateFilter} GROUP BY status`,
        queryParams
      );

      // Get meetings by type
      const [meetingsByTypeResult] = await db.query(
      ...
      // Get busiest days
      const [busiestDaysResult] = await db.query(
      ...
      // Get busiest hours
      const [busiestHoursResult] = await db.query(
  ...
  getUserMeetingAnalytics: async (req, res) => {
    try {
      ...
      if (currentUserID !== parseInt(userId) && req.user.roleID !== 4) { // Assuming roleID 4 is admin
